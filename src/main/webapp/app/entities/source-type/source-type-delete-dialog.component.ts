...
export class SourceTypeDeleteDialogComponent {
    ...
    confirmDelete(id: number) {
        this.sourceTypeService.delete(id).subscribe(response => {
...
export class SourceTypeDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ sourceType }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(SourceTypeDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
                this.ngbModalRef.componentInstance.sourceType = sourceType;
